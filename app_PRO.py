...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Búsqueda de Fecha robusta: "Nº factura: ... emitida el DD de MES de AAAA"
        m_fecha_xxi = re.search(r'Nº\s+factura:.*emitida\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})', texto_completo, re.IGNORECASE)
        fecha = m_fecha_xxi.group(1) if m_fecha_xxi else "No encontrada"

        # Búsqueda de Total Real: Suma de Potencia contratada + Energía consumida
        # Se usa una expresión que limpia comillas y espacios
        ...
        v_pot = float(m_val_pot_xxi.group(1).replace(',', '.')) if m_val_pot_xxi else 0.0
        v_ene = float(m_val_ene_xxi.group(1).replace(',', '.')) if m_val_ene_xxi else 0.0
        total_real = v_pot + v_ene

        # Resto de valores necesarios para el diccionario final
        patrones_consumo = {
