...
# --- SECCIÓN DE EXPORTACIÓN ---
            ...
            col1.download_button(
                ...
                file_name=f"comparativa_energia_{mejor['Mes/Fecha'].replace('/', '-')}.xlsx",
            ...
            class PDF(FPDF):
                def header(self):
                    ...
                    self.cell(0, 10, 'Informe de Ahorro Energético', 0, 1, 'C')
                    self.ln(5)
            ...
            pdf.cell(0, 10, f"Periodo analizado: {mejor['Mes/Fecha']}", 0, 1)
            pdf.ln(5)

            # Resumen destacado
            if mejor["Ahorro"] > 0:
                pdf.set_fill_color(200, 255, 200) # Fondo verde claro
                pdf.set_font("Arial", 'B', 12)
                pdf.cell(0, 10, f"OPCIÓN RECOMENDADA: {mejor['Compañía/Tarifa']}", 1, 1, 'C', True)
                pdf.set_font("Arial", size=12)
                pdf.cell(0, 10, f"Ahorro en este recibo: {mejor['Ahorro']} EUR", 0, 1)
                pdf.cell(0, 10, f"Ahorro anual estimado: {round(ahorro_anual, 2)} EUR", 0, 1)
            
            pdf.ln(10)
            pdf.set_font("Arial", 'B', 10)
            # Cabecera de tabla
            pdf.cell(80, 10, 'Compañia/Tarifa', 1)
            pdf.cell(40, 10, 'Coste', 1)
            pdf.cell(40, 10, 'Diferencia', 1)
            pdf.ln()
            ...
            col2.download_button(
                ...
                data=pdf_output,
                file_name=f"informe_ahorro_{mejor['Mes/Fecha'].replace('/', '-')}.pdf",
