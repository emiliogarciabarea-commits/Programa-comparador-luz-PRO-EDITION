...
def extraer_datos_factura(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pagina in pdf.pages:
            ...
            if texto_pag:
                ...
                # Limpiamos cada línea de espacios extraños al leer
    ...
    if es_el_corte_ingles:
        ...
        # --- BLOQUE ENDESA LUZ: BUSQUEDA ROBUSTA ---
        fecha = "No encontrada"
        dias = 0
        potencia = 0.0
        ...
        for i, linea in enumerate(lineas_factura):
            # 1. FECHA DE EMISIÓN: Buscamos la etiqueta y extraemos la primera fecha que aparezca cerca
            if "emisión factura" in linea.lower():
                # Buscamos en la misma línea o en la siguiente por si hay salto
                bloque = linea + " " + (lineas_factura[i+1] if i+1 < len(lineas_factura) else "")
                m_f = re.search(r'(\d{2}/\d{2}/\d{4})', bloque)
                if m_f: fecha = m_f.group(1)

            # 2. DÍAS: Formato "(27 días)"
            ...
            if "P1" in linea and "kW" in linea:
                ...
                if m_p: potencia = float(m_p.group(1).replace(',', '.'))

            # 4. CONSUMOS POR PERIODOS
            ...
            # 5. IMPORTES PARA EL TOTAL (RESUMEN)
            if "Potencia" in linea and "€" in linea and "Factura" not in linea:
            ...
            for _, fact in df_resumen_pdfs.iterrows():
                ...
                for index, tarifa in df_tarifas.iterrows():
                    try:
                        ...
                        resultados_finales.append({"Mes/Fecha": fact['Fecha'], "Compañía/Tarifa": nombre_cia, "Coste (€)": round(coste_estimado, 2), "Ahorro": round(fact['Total Real'] - coste_estimado, 2)})
                    except: continue
