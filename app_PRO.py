...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # --- CORRECCIÓN FECHA ENERGÍA XXI (Uso de Fecha de Emisión por ser más estándar) ---
        ...
        # --- CORRECCIÓN TOTAL REAL ENERGÍA XXI (Búsqueda más robusta ante comillas/saltos) ---
        m_val_pot_xxi = re.search(r'potencia\s+contratada.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        m_val_ene_xxi = re.search(r'energía\s+consumida.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
