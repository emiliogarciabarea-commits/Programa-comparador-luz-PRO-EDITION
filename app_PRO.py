...
def extraer_datos_factura(pdf_path):
    ...
    es_eci = re.search(r'Energía El Corte Inglés|TELECOR', texto_completo, re.IGNORECASE)
    ...
    # Lógica específica para capturar consumos de la tabla de ECI si fallan los patrones genéricos
    if es_eci:
        patrones_consumo['punta'].append(r'Consumo total \[kWh\]\s+([\d,.]+)\s+[\d,.]+\s+[\d,.]+')
        patrones_consumo['llano'].append(r'Consumo total \[kWh\]\s+[\d,.]+\s+([\d,.]+)\s+[\d,.]+')
        patrones_consumo['valle'].append(r'Consumo total \[kWh\]\s+[\d,.]+\s+[\d,.]+\s+([\d,.]+)')
    ...
    patron_potencia = r'(?:Potencia\s+contratada(?:\s+en\s+punta-llano|\s+P1)?):\s*([\d,.]+)\s*kW|Punta:\s*([\d,.]+)\s*kW'
    match_potencia = re.search(patron_potencia, texto_completo, re.IGNORECASE)
    if match_potencia:
        val = match_potencia.group(1) if match_potencia.group(1) else match_potencia.group(2)
        potencia = float(val.replace(',', '.'))
    else:
        potencia = 0.0

    # 3. Fecha y Días
    patron_fecha = r'(?:emitida\s+el|Fecha\s+de\s+Factura:)\s*([\d/]{8,10})'
    ...
    if match_dias:
        # Intenta capturar si el número está después de los dos puntos o antes de la palabra días
        dias_val = re.search(r'Días\s+de\s+consumo:\s*(\d+)', texto_completo)
        dias = int(dias_val.group(1)) if dias_val else int(match_dias.group(1))
    else:
        dias = 0
    ...
    # --- Lógica específica de Factura Actual (Total) ---
    total_real = 0.0
