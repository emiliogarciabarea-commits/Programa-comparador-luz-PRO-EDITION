...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        def extraer_kwh(tipo, texto):
            matches = re.findall(rf'{tipo}.*?([\d,.]+)\s*kWh', texto, re.IGNORECASE)
        ...
        dias = int(re.search(r'Financiación\s+de\s+Bono\s+Social\s+(\d+)\s+días', texto_completo, re.IGNORECASE).group(1)) if re.search(r'Financiación\s+de\s+Bono\s+Social\s+(\d+)\s+días', texto_completo, re.IGNORECASE) else 0
        ...
        total_real = float(re.search(r'Subtotal\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE).group(1).replace(',', '.')) if re.search(r'Subtotal\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE) else 0.0
        ...
        dias = int(re.search(r'(\d+)\s+días', texto_completo, re.IGNORECASE).group(1)) if re.search(r'(\d+)\s+días', texto_completo, re.IGNORECASE) else 0
        potencia = float(re.search(r'punta-llano\s*([\d,.]+)\s*kW', texto_completo, re.IGNORECASE).group(1).replace(',', '.')) if re.search(r'punta-llano\s*([\d,.]+)\s*kW', texto_completo, re.IGNORECASE) else 0.0
        m_val_pot = re.search(r'Potencia\s+\.+\s*([\d\s.,]+)€', texto_completo, re.IGNORECASE)
        m_val_ene = re.search(r'Energía\s+\.+\s*([\d\s.,]+)€', texto_completo, re.IGNORECASE)
        total_real = (float(m_val_pot.group(1).replace(" ", "").replace(".", "").replace(",", ".")) if m_val_pot else 0.0) + (float(m_val_ene.group(1).replace(" ", "").replace(".", "").replace(",", ".")) if m_val_ene else 0.0)
        ...
        total_real = (float(re.search(r'Término\s+fijo\s*([\d,.]+)\s*€', texto_completo).group(1).replace(',', '.')) if re.search(r'Término\s+fijo\s*([\d,.]+)\s*€', texto_completo) else 0.0) + (float(re.search(r'Energía\s*([\d,.]+)\s*€', texto_completo).group(1).replace(',', '.')) if re.search(r'Energía\s*([\d,.]+)\s*€', texto_completo) else 0.0)
        ...
        m_periodo = re.search(r'PERIODO\s+DE\s+FACTURACIÓN:?.*?(\d{2}/\d{2}/\d{2,4}).*?(\d{2}/\d{2}/\d{2,4})', texto_completo, re.IGNORECASE | re.DOTALL)
        ...
        # Búsqueda de Fecha de cargo específica para Energía XXI
        m_fecha_xxi = re.search(r'Fecha\s+de\s+cargo:\s*([\d]{1,2}\s+de\s+\w+\s+de\s+\d{4})', texto_completo, re.IGNORECASE)
        fecha = m_fecha_xxi.group(1) if m_fecha_xxi else "No encontrada"
        
        # Extracción de consumos (Punta, Llano, Valle)
        patrones_consumo = {'punta': [r'P1:?\s*([\d,.]+)\s*kWh', r'Punta\s*([\d,.]+)\s*kWh'], 'llano': [r'P2:?\s*([\d,.]+)\s*kWh', r'Llano\s*([\d,.]+)\s*kWh'], 'valle': [r'P3:?\s*([\d,.]+)\s*kWh', r'Valle\s*([\d,.]+)\s*kWh']}
        ...
        # Lógica de "Total Real" para Energía XXI: Suma de término fijo + término variable
        # Buscamos la fila "TÉRMINO FIJO" y el valor en € que le sigue (posibles saltos de línea/puntos)
        m_fijo_xxi = re.search(r'TÉRMINO\s+FIJO.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        m_variable_xxi = re.search(r'TÉRMINO\s+VARIABLE.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        
        v_fijo = float(m_fijo_xxi.group(1).replace('.', '').replace(',', '.')) if m_fijo_xxi else 0.0
        v_variable = float(m_variable_xxi.group(1).replace('.', '').replace(',', '.')) if m_variable_xxi else 0.0
        total_real = v_fijo + v_variable

    else:
        # Lógica por defecto (la que tenías originalmente en el else)
        patrones_consumo = {'punta': [r'P1:?\s*([\d,.]+)\s*kWh', r'Punta\s*([\d,.]+)\s*kWh'], 'llano': [r'P2:?\s*([\d,.]+)\s*kWh', r'Llano\s*([\d,.]+)\s*kWh'], 'valle': [r'P3:?\s*([\d,.]+)\s*kWh', r'Valle\s*([\d,.]+)\s*kWh']}
        for tramo, patrones in patrones_consumo.items():
            for patron in patrones:
                ...
                if match:
                    ...
                    break
        potencia = float(re.search(r'([\d,.]+)\s*kW', texto_completo).group(1).replace(',', '.')) if re.search(r'([\d,.]+)\s*kW', texto_completo) else 0.0
        fecha = re.search(r'Fecha\s+de\s+cargo:\s*([\d]{1,2}\s+de\s+\w+\s+de\s+\d{4})', texto_completo, re.IGNORECASE).group(1) if re.search(r'Fecha\s+de\s+cargo:\s*([\d]{1,2}\s+de\s+\w+\s+de\s+\d{4})', texto_completo, re.IGNORECASE) else "No encontrada"
        dias = int(re.search(r'(\d+)\s*días', texto_completo).group(1)) if re.search(r'(\d+)\s*días', texto_completo) else 0
