...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # 5, 6 y 7. Consumos (Patrones mejorados para Repsol)
        # Buscamos la palabra y el número que le sigue, permitiendo espacios o "kWh"
        m_punta = re.search(r'Punta\s*([\d,.]+)', texto_completo, re.IGNORECASE)
        m_llano = re.search(r'Llano\s*([\d,.]+)', texto_completo, re.IGNORECASE)
        m_valle = re.search(r'Valle\s*([\d,.]+)', texto_completo, re.IGNORECASE)
...
# El resto del código Streamlit permanece idéntico...
