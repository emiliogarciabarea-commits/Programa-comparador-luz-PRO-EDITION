...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # 1. Fecha: Formato DD.MM.AAAA [cite: 5]
        m_fecha = re.search(r'Fecha\s+emisión:\s*(\d{2}\.\d{2}\.\d{4})', texto_completo, re.IGNORECASE)
        fecha = m_fecha.group(1) if m_fecha else "No encontrada"

        # 2. Días: Extraído de la tabla de potencia [cite: 81]
        m_dias = re.search(r'Período\s+(\d+)\s+dia\(s\)', texto_completo, re.IGNORECASE)
        dias = int(m_dias.group(1)) if m_dias else 0

        # 3. Potencia: P1 o P2 (suelen ser iguales) [cite: 81]
        m_pot = re.search(r'Potencia\s+P1:\s*([\d,.]+)', texto_completo, re.IGNORECASE)
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0

        # 4. Consumos: Detalle por tramos punta, llano y valle [cite: 91]
        ...
        consumos = {
            ...
            'valle': float(m_valle.group(2).replace(',', '.')) if m_valle else 0.0
        }
        
        # 5. Importe Total [cite: 87]
