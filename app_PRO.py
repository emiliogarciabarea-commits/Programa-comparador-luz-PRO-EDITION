...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        patron_fecha = r'(?:emitida\s+el|Fecha\s+de\s+emisión:)\s*([\d/]+)'
        ...
        match_excedente = re.search(r'excedentes\s*.*?(-?\d+[\d,.]*)\s*kWh', texto_completo, re.IGNORECASE)
        excedente = abs(float(match_excedente.group(1).replace(',', '.'))) if match_excedente else 0.0
        
        m_total_xxi = re.search(r'Total\s+Factura\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        total_real = float(m_total_xxi.group(1).replace(',', '.')) if m_total_xxi else 0.0
    ...
    if uploaded_files:
        ...
        if datos_facturas:
            ...
            with pd.ExcelWriter(buffer_excel, engine='openpyxl') as writer:
                ...
                df_resumen_pdfs.drop(columns=["Compañía"], errors='ignore').to_excel(writer, index=False, sheet_name='Datos Facturas Originales')
