...
def extraer_datos_factura(pdf_path):
    ...
    if es_endesa_luz:
        ...
        # 5. Total Real (Suma de Potencia + Energia con puntos suspensivos)
        val_pot = 0.0
        val_ene = 0.0
        for linea in lineas_factura:
            # Captura el importe tras los puntos suspensivos
    ...
    # --- RESTO DEL CÓDIGO ORIGINAL (SIN MODIFICAR) ---
        ...
        val_c = float(m_consumo_gen.group(1).replace(',', '.')) if m_consumo_gen else 0.0
        consumos = {'punta': val_c, 'llano': 0.0, 'valle': 0.0}
        excedente = 0.0
        ...
        fecha = m_fecha.group(1) if m_fecha else "No encontrada"
        if es_naturgy:
            m_d_nat = re.search(r'Término\s+potencia\s+P1.*?(\d+)\s+días', texto_completo, re.IGNORECASE | re.DOTALL)
            dias = int(m_d_nat.group(1)) if m_d_nat else 0
        else:
            m_dias = re.search(r'(\d+)\s*días', texto_completo)
            dias = int(m_dias.group(1)) if m_dias else 0
        ...
        m_total = re.search(r'(?:Subtotal|Importe\s+total|Total\s+factura)\s*:?\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        total_real = float(m_total.group(1).replace(',', '.')) if m_total else 0.0
            ...
            for _, fact in df_resumen_pdfs.iterrows():
                ...
                for index, tarifa in df_tarifas.iterrows():
                    try:
                        ...
                        coste_est = (fact['Días'] * b_pot1 * fact['Potencia (kW)']) + \
                        ...
                        ahorro = fact['Total Real'] - coste_est
                        resultados_finales.append({
                            "Mes/Fecha": fact['Fecha'], "Compañía/Tarifa": nombre_cia,
                            "Coste (€)": round(coste_est, 2), "Ahorro": round(ahorro, 2)
                        })
                    except: continue
