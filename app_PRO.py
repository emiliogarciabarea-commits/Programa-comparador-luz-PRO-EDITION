...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        if m_subtotal: total_real = float(m_subtotal.group(1).replace(',', '.'))
        else:
        ...
        m_punta = re.search(r'Punta\s*([\d,.]+)\s*kWh', texto_completo); m_llano = re.search(r'Llano\s*([\d,.]+)\s*kWh', texto_completo); m_valle = re.search(r'Valle\s*([\d,.]+)\s*kWh', texto_completo)
        consumos = {'punta': float(m_punta.group(1).replace(',', '.')) if m_punta else 0.0, 'llano': float(m_llano.group(1).replace(',', '.')) if m_llano else 0.0, 'valle': float(m_valle.group(1).replace(',', '.')) if m_valle else 0.0}
        ...
        # --- BLOQUE DE LIMPIEZA CRÍTICA PARA ENERGÍA XXI ---
        # Unimos todo el texto y eliminamos saltos de línea para que la fecha no se "esconda"
        t_norm = " ".join(texto_completo.split())
        
        # 1. FECHA (Búsqueda ultra-flexible)
        # Busca 'emitida el' + cualquier cosa hasta encontrar un año de 4 cifras
        m_fecha = re.search(r'emitida\s+el\s+([\d]{1,2}.*?\d{4})', t_norm, re.IGNORECASE)
        fecha = m_fecha.group(1) if m_fecha else "No encontrada"

        # 2. CONSUMOS (Limpiando comillas de pdfplumber)
        ...
        for tramo, patrones in patrones_consumo.items():
            ...
            for patron in patrones:
                match = re.search(patron, t_norm, re.IGNORECASE)
                if match:
                    consumos[tramo] = float(match.group(1).replace(',', '.'))
                    break
        
        # 3. OTROS DATOS
        m_pot = re.search(r'([\d,.]+)\s*kW', t_norm)
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
        m_dias = re.search(r'(\d+)\s*días', t_norm)
        dias = int(m_dias.group(1)) if m_dias else 0
        m_exc = re.search(r'Valoración\s+excedentes.*?(-?\d+[\d,.]*)\s*kWh', t_norm, re.IGNORECASE)
        excedente = abs(float(m_exc.group(1).replace(',', '.'))) if m_exc else 0.0
        
        # 4. TOTAL REAL (Suma Potencia + Energía)
        m_val_pot = re.search(r'potencia\s+contratada\s*\"?,?\s*([\d,.]+)\s*€', t_norm, re.IGNORECASE)
        m_val_ene = re.search(r'energía\s+consumida\s*\"?,?\s*([\d,.]+)\s*€', t_norm, re.IGNORECASE)
        if m_val_pot and m_val_ene:
            total_real = float(m_val_pot.group(1).replace(',', '.')) + float(m_val_ene.group(1).replace(',', '.'))
        else:
            m_tot = re.search(r'Total\s+electricidad\s*([\d,.]+)\s*€', t_norm, re.IGNORECASE)
    ...
    if uploaded_files:
        ...
        if datos_facturas:
            ...
            for _, fact in df_resumen_pdfs.iterrows():
                resultados_finales.append({"Mes/Fecha": fact['Fecha'], "Compañía/Tarifa": f"📍 TU FACTURA ACTUAL", "Coste (€)": fact['Total Real'], "Ahorro": 0.0})
                for _, tarifa in df_tarifas.iterrows():
                    try:
                        coste = (fact['Días'] * tarifa.iloc[1] * fact['Potencia (kW)']) + (fact['Días'] * tarifa.iloc[2] * fact['Potencia (kW)']) + (fact['Consumo Punta (kWh)'] * tarifa.iloc[3]) + (fact['Consumo Llano (kWh)'] * tarifa.iloc[4]) + (fact['Consumo Valle (kWh)'] * tarifa.iloc[5]) - (fact['Excedente (kWh)'] * tarifa.iloc[6])
                        resultados_finales.append({"Mes/Fecha": fact['Fecha'], "Compañía/Tarifa": tarifa.iloc[0], "Coste (€)": round(coste, 2), "Ahorro": round(fact['Total Real'] - coste, 2)})
                    except: continue
