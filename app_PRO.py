...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Búsqueda de Fecha: Nº factura: S2... emitida el DD de MES de AAAA
        m_fecha_xxi = re.search(r'Nº\s+factura:.*?\s+emitida\s+el\s+(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})', texto_completo, re.IGNORECASE)
        fecha = m_fecha_xxi.group(1) if m_fecha_xxi else "No encontrada"
        
        # Búsqueda de "Total Real" (Término Fijo + Variable)
        # Se buscan valores tipo XX,XX € después de las etiquetas mencionadas ignorando saltos de línea
        m_fijo_xxi = re.search(r'TÉRMINO\s+FIJO.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        m_vari_xxi = re.search(r'TÉRMINO\s+VARIABLE.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        
        v_fijo = float(m_fijo_xxi.group(1).replace(',', '.')) if m_fijo_xxi else 0.0
        v_vari = float(m_vari_xxi.group(1).replace(',', '.')) if m_vari_xxi else 0.0
        total_real = v_fijo + v_vari

        # Resto de datos para Energía XXI
        patrones_consumo = {
        ...
        m_pot_xxi = re.search(r'([\d,.]+)\s*kW', texto_completo)
        potencia = float(m_pot_xxi.group(1).replace(',', '.')) if m_pot_xxi else 0.0
        m_dias_xxi = re.search(r'(\d+)\s*días', texto_completo)
        dias = int(m_dias_xxi.group(1)) if m_dias_xxi else 0
        m_exc_xxi = re.search(r'Valoración\s+excedentes.*?(-?[\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        excedente = abs(float(m_exc_xxi.group(1).replace(',', '.'))) if m_exc_xxi else 0.0

    else:
        # Lógica para otras compañías desconocidas
        patrones_consumo = {
            'punta': [r'P1:?\s*([\d,.]+)\s*kWh', r'Punta\s*([\d,.]+)\s*kWh'],
            'llano': [r'P2:?\s*([\d,.]+)\s*kWh', r'Llano\s*([\d,.]+)\s*kWh'],
            'valle': [r'P3:?\s*([\d,.]+)\s*kWh', r'Valle\s*([\d,.]+)\s*kWh']
        ...
        m_val_pot_gen = re.search(r'Por\s+potencia\s+contratada\s*[\n\r]*\s*\"?,?\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        m_val_ene_gen = re.search(r'Por\s+energía\s+consumida\s*[\n\r]*\s*\"?,?\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        
        if m_val_pot_gen and m_val_ene_gen:
            total_real = float(m_val_pot_gen.group(1).replace(',', '.')) + float(m_val_ene_gen.group(1).replace(',', '.'))
        else:
