...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Búsqueda flexible de fecha: DD de mes de AAAA
        patron_fecha_xxi = r'Fecha\s+de\s+cargo:\s*(\d{1,2}\s+de\s+\w+\s+de\s+\d{4})'
        m_fecha = re.search(patron_fecha_xxi, texto_completo, re.IGNORECASE)
        fecha = m_fecha.group(1) if m_fecha else "No encontrada"

        # Búsqueda de consumos P1, P2, P3
        patrones_consumo = {
        ...
        # Total Real: Suma de Potencia + Energía (flexible a saltos de línea y comillas de tablas)
        m_val_pot = re.search(r'Por\s+potencia\s+contratada[\s\S]*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        m_val_ene = re.search(r'Por\s+energía\s+consumida[\s\S]*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        ...
        # Excedentes
        m_exc = re.search(r'Valoración\s+excedentes.*?(-?\d+[\d,.]*)\s*kWh', texto_completo, re.IGNORECASE)
        ...
        m_fecha_et = re.search(r'Fecha\s+emisión\s+factura:\s*([\d/]{10})', texto_completo, re.IGNORECASE)
        fecha = m_fecha_et.group(1) if m_fecha_et else "No encontrada"
        ...
        def limpia_e(patron, txt):
            m = re.search(patron, txt, re.IGNORECASE)
            return float(m.group(1).replace(" ", "").replace(".", "").replace(",", ".")) if m else 0.0

        total_real = limpia_e(r'Potencia\s+\.+\s*([\d\s.,]+)€', texto_completo) + limpia_e(r'Energía\s+\.+\s*([\d\s.,]+)€', texto_completo)
        m_punta = re.search(r'Punta\s+[\d,.]+\s+[\d,.]+\s+[\d,.]+\s+[\w,.]+\s+([\d,.]+)', texto_completo)
        ...
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
        m_periodo = re.search(r'PERIODO\s+DE\s+FACTURACIÓN:?.*?(\d{2}/\d{2}/\d{2,4})', texto_completo, re.IGNORECASE | re.DOTALL)
        fecha = m_periodo.group(1) if m_periodo else "No encontrada"
