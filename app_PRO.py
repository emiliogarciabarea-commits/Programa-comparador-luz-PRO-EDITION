...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # --- SECCIÓN ULTRA ROBUSTA ENERGÍA XXI ---
        if es_xxi: compania = "Energía XXI"
        # Fecha de cargo (Permite saltos de línea y captura el formato fecha español)
        m_f = re.search(r'cargo:?\s*([\d]{1,2}\s+de\s+\w+\s+de\s+[\d]{4})', texto_completo, re.IGNORECASE)
        fecha = m_f.group(1) if m_f else "No encontrada"
        
        # Potencia y Energía (Suma real) - Busca el label y el siguiente número ignorando todo lo demás
        m_p = re.search(r'potencia\s+contratada\s+([\d,.]+)', texto_completo, re.IGNORECASE)
        m_e = re.search(r'energ[íiaá]\s+consumida\s+([\d,.]+)', texto_completo, re.IGNORECASE)
        
        if m_p and m_e:
            total_real = float(m_p.group(1).replace(',', '.')) + float(m_e.group(1).replace(',', '.'))
        else:
            m_t = re.search(r'TOTAL\s+IMPORTE\s+FACTURA\s+([\d,.]+)', texto_completo, re.IGNORECASE)
            total_real = float(m_t.group(1).replace(',', '.')) if m_t else 0.0

        # Otros datos básicos
        m_pot = re.search(r'([\d,.]+)\s*kW', texto_completo)
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
        m_d = re.search(r'(\d+)\s*días', texto_completo, re.IGNORECASE)
        dias = int(m_d.group(1)) if m_d else 0
        consumos = {'punta': 0.0, 'llano': 0.0, 'valle': 0.0}
        for k, p in {'punta': r'P1:?\s*([\d,.]+)', 'llano': r'P2:?\s*([\d,.]+)', 'valle': r'P3:?\s*([\d,.]+)'}.items():
...
if not os.path.exists(excel_path): st.error(f"No se encuentra el archivo '{excel_path}'")
else:
    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            except Exception as e: st.error(f"Error procesando {uploaded_file.name}: {e}")
        if datos_facturas:
            ...
            for _, fact in df_resumen_pdfs.iterrows():
                resultados_finales.append({"Mes/Fecha": fact['Fecha'], "Compañía/Tarifa": f"📍 TU FACTURA ACTUAL", "Coste (€)": fact['Total Real'], "Ahorro": 0.0, "Dias_Factura": fact['Días']})
            ...
            ranking_total = df_comp[~df_comp["Compañía/Tarifa"].str.contains("📍 TU FACTURA")].groupby("Compañía/Tarifa")["Ahorro"].sum().reset_index().sort_values(by="Ahorro", ascending=False)
            st.divider()
            if not ranking_total.empty:
                st.success(f"La mejor compañía es: **{ranking_total.iloc[0]['Compañía/Tarifa']}**")
            ...
            st.download_button(label="📥 Descargar Informe Completo", data=buffer_excel.getvalue(), file_name="comparativa.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
