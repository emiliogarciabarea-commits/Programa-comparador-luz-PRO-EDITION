...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Búsqueda de Fecha de cargo (DD de MES de AAAA)
        patron_fecha = r'Fecha\s+de\s+cargo:\s*([\d]{1,2}\s+de\s+\w+\s+de\s+\d{4})'
        match_fecha = re.search(patron_fecha, texto_completo, re.IGNORECASE)
        fecha = match_fecha.group(1) if match_fecha else "No encontrada"

        # Búsqueda de consumos y potencia (mantenemos lógica general)
        patrones_consumo = {
        ...
        # --- CÁLCULO TOTAL REAL ESPECÍFICO ---
        # Buscamos "Por potencia contratada" y "Por energía consumida", saltando posibles comillas, comas iniciales y saltos de línea
        ...
        if m_val_pot_xxi and m_val_ene_xxi:
            ...
            m_total_respaldo = re.search(r'Total\s+electricidad\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
            total_real = float(m_total_respaldo.group(1).replace(',', '.')) if m_total_respaldo else 0.0

    else:
        # Lógica genérica para otras compañías
...
# --- Código Streamlit (Permanece igual) ---
