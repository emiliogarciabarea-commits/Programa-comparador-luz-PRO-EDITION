...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # --- LÓGICA INTELIGENTE PARA ENDESA LUZ ---
        fecha = "No encontrada"
        dias = 0
        potencia = 0.0
        ...
        # Recorremos líneas para encontrar la fecha por proximidad
        for i, linea in enumerate(lineas_factura):
            if "Fecha emisión factura" in linea:
                # Intenta buscar la fecha en la misma línea o en las 2 siguientes
                bloque_fecha = " ".join(lineas_factura[i:i+3])
                m_f = re.search(r'(\d{2}/\d{2}/\d{4})', bloque_fecha)
                if m_f: fecha = m_f.group(1)
            
            if "días" in linea.lower() and "(" in linea:
            ...
            # Captura de consumos por etiquetas
