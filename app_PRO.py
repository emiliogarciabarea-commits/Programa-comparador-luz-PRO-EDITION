...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Consumos P1, P2, P3
        m_p1 = re.search(r'(?:Consumo\s+en\s+P1|Punta).*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        m_p2 = re.search(r'(?:Consumo\s+en\s+P2|Llano).*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        m_p3 = re.search(r'(?:Consumo\s+en\s+P3|Valle).*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        consumos = {
            'punta': float(m_p1.group(1).replace(',', '.')) if m_p1 else 0.0,
            'llano': float(m_p2.group(1).replace(',', '.')) if m_p2 else 0.0,
            'valle': float(m_p3.group(1).replace(',', '.')) if m_p3 else 0.0
        }
        ...
        m_total = re.search(r'Importe\s+total\s+factura.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        ...
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
        m_punta = re.search(r'Punta.*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        m_llano = re.search(r'Llano.*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        m_valle = re.search(r'Valle.*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        consumos = {
            'punta': float(m_punta.group(1).replace(',', '.')) if m_punta else 0.0,
            'llano': float(m_llano.group(1).replace(',', '.')) if m_llano else 0.0,
            'valle': float(m_valle.group(1).replace(',', '.')) if m_valle else 0.0
        ...
        m_fecha = re.search(r'Fecha\s+de\s+emisión\s*([\d/]+)', texto_completo, re.IGNORECASE)
        fecha = m_fecha.group(1) if m_fecha else "No encontrada"
        m_pot = re.search(r'Potencia\s+contratada\s*([\d,.]+)', texto_completo, re.IGNORECASE)
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
        m_dias = re.search(r'Días\s+facturados\s*(\d+)', texto_completo, re.IGNORECASE)
        dias = int(m_dias.group(1)) if m_dias else 0
        m_consumo = re.search(r'Consumo\s+.*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
        consumos['punta'] = float(m_consumo.group(1).replace(',', '.')) if m_consumo else 0.0
        m_total = re.search(r'Importe\s+total\s+factura.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        total_real = float(m_total.group(1).replace(',', '.')) if m_total else 0.0

    elif es_iberdrola:
        compania = "Iberdrola"
        m_pot = re.search(r'Potencia\s+punta:\s*([\d,.]+)', texto_completo, re.IGNORECASE)
        potencia = float(m_pot.group(1).replace(',', '.')) if m_pot else 0.0
    ...
    if uploaded_files:
        ...
        if datos_facturas:
            ...
            if not ranking_total.empty:
                ...
                c1, c2 = st.columns(2)
                with c1: st.success(f"La mejor compañía es: **{ranking_total.iloc[0]['Compañía/Tarifa']}**")
            ...
            with pd.ExcelWriter(buffer_excel, engine='openpyxl') as writer:
                df_comp.drop(columns=['p1', 'p2', 'ep', 'el', 'ev', 'exc'], errors='ignore').to_excel(writer, index=False, sheet_name='Detalle Comparativa')
                ranking_total.to_excel(writer, index=False, sheet_name='Ranking Ahorro')
                df_resumen_pdfs.drop(columns=["Compañía"]).to_excel(writer, index=False, sheet_name='Datos Facturas Originales')
                if not ranking_total.empty: df_ganadora.to_excel(writer, index=False, sheet_name='Precios Tarifa Ganadora')

            st.download_button(label="📥 Descargar Informe Completo", data=buffer_excel.getvalue(), file_name="estudio_ahorro_energetico.xlsx", use_container_width=True)
