...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Fecha de emisión para Energía XXI
        patron_fecha = r'emitida\s+el\s+([\d]{1,2}\s+de\s+\w+\s+de\s+\d{4}|[\d/]{8,10})'
        ...
        # Lectura flexible de Potencia + Energía para Total Real
        m_val_pot_xxi = re.search(r'Por\s+potencia\s+contratada\s*[\"\'\s,]*\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        m_val_ene_xxi = re.search(r'Por\s+energía\s+consumida\s*[\"\'\s,]*\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
