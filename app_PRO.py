...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Normalización de texto para Energía XXI (Elimina ruidos de espacios y saltos)
        ...
        match_potencia = re.search(patron_potencia, texto_norm)
        potencia = float(match_potencia.group(1).replace(',', '.')) if match_potencia else 0.0
        
        # Búsqueda de fecha mejorada para Energía XXI (Independiente de espacios invisibles)
        patron_fecha = r'emitida\s+el\s+([\d]{1,2}.*?\d{4})'
        match_fecha = re.search(patron_fecha, texto_norm, re.IGNORECASE)
        fecha = match_fecha.group(1) if match_fecha else "No encontrada"
        
        match_dias = re.search(r'(\d+)\s*días', texto_norm)
        dias = int(match_dias.group(1)) if match_dias else 0
        
        match_excedente = re.search(r'Valoración\s+excedentes\s*(?:-?\d+[\d,.]*\s*€/kWh)?\s*(-?\d+[\d,.]*)\s*kWh', texto_norm, re.IGNORECASE)
        ...
        if m_val_pot_xxi and m_val_ene_xxi:
            ...
            match_total = re.search(r'Total\s+electricidad\s*([\d,.]+)\s*€', texto_norm, re.IGNORECASE)
            total_real = float(match_total.group(1).replace(',', '.')) if match_total else 0.0
